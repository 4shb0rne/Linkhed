...
func (server *Server) RegisterGoogle(w http.ResponseWriter, r *http.Request) {
...
func (server *Server) GetSuggestedUser(w http.ResponseWriter, r *http.Request) {
	...
	user := models.User{}
	userGotten, err := user.FindSuggestedUser(server.DB, uint32(uid))
	if err != nil {
		responses.ERROR(w, http.StatusBadRequest, err)
		return
	}
